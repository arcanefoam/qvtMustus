/*******************************************************************************
 * Copyright (c) 2005 E.D.Willink and others.
...
//	public abstract Orphanage getOrphanage();	
	...
	public String toString() {
		return getClass().getSimpleName() + "[" + DebugString.toDebug(getResource()) + "]"
...
//			workspace.addResourceChangeListener(this, IResourceChangeEvent.POST_CHANGE);
		...
		public boolean computeIsReadOnly() {
			...
			try {
				...
				QVTEditorPlugin.logError("Failed to access " + uri.toString(), e);
			}
			return true;
		}
...
//		}
		...
		protected void handleNotification(Notification msg) {
			...
			if (EcoreUtils.getFeatureID(msg, resource, Resource.class) == Resource.RESOURCE__ERRORS) {
				...
				QVTEditorPlugin.logError("Error in '" + resource.getURI() + "'",  e instanceof Throwable ? (Throwable)e : null);
			}
		}
...
//			System.out.println("Resource changed : '" + pathString + "' wrt '" + fileName + "'");
		...
		public void setLocked(boolean setLocked) {
			if (setLocked) {
				if (resource.isModified()) {
					QVTEditorPlugin.showError("Cannot lock modified resource: " + DebugString.toDebug(resource), null);
					return;
				}
			}
			else {
				if (computeIsReadOnly()) {
					QVTEditorPlugin.showError("Cannot unlock read-only resource: " + DebugString.toDebug(resource), null);
					return;
				}
			}
		...
		public void setReadOnly(boolean setReadOnly) {
			if (setReadOnly) {
				if (resource.isModified()) {
					QVTEditorPlugin.showError("Cannot set modified resource read-only: " + DebugString.toDebug(resource), null);
					return;
				}
			}
			else {
				if (computeIsReadOnly()) {
					QVTEditorPlugin.showError("Cannot set read-only resource read-write: " + DebugString.toDebug(resource), null);
					return;
				}
			}
